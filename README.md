...
\| \`context\_manager.py\` | Prepara o contexto de execução antes dos testes. |

\| \`page\_validator.py\` | Identifica padrões de erro e extrai suas mensagens. |
...
SCREENSHOT\_PATH=screenshots

SCREENSHOT\_MODE=ERROR
...
\| \`MAX\_APPS=0\` | Testa todas as aplicações. |

\| \`MAX\_APPS=N\` | Limita a execução às primeiras \`N\` aplicações. |

\| \`ORDER=ASC\` | Ordena as aplicações de forma crescente. |

\| \`ONLY\_ENABLED=True\` | Considera somente aplicações habilitadas. |
...
https\://servidor/app/tela/tela.php?var\_codiempr=15&var\_codibene=8&var\_codicolab=25
